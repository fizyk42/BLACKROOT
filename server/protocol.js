/**
 * protocol — BLACKROOT co-op + public online room protocol.
 */
...
export function doHello(hub, conn, m, addr, log = () => {}) {
  ...
  if (m.public === true || String(m.code || '').toUpperCase() === 'PUBLIC') {
    room = hub.matchmake(m.seed, m.route);
    if (!room) { conn.send(JSON.stringify({ t: 'err', code: 'busy', m: 'Public matchmaking is full right now. Try again shortly.' })); conn.close(4004, 'no rooms'); return null; }
  } else if (m.host) {
  ...
  if (room.full) {
    conn.send(JSON.stringify({ t: 'err', code: 'full', m: `That session is full (${LIMITS.players} players).` }));
    conn.close(4005, 'full');
    return null;
  }
  ...
  log(`${player.name} joined ${room.public ? 'PUBLIC ' : ''}${room.code} from ${addr} (${room.size} in room)`);
  return player;
}
...
export function handle(p, m) {
  ...
  switch (m.t) {
    ...
    case 'bosshit': {
      ...
      const phase = room.boss.health > room.boss.maxHealth * 0.66 ? 1 : room.boss.health > room.boss.maxHealth * 0.33 ? 2 : 3;
    ...
    case 'world': {
      ...
      if (m.boss === null) room.boss = null;
        ...
        room.boss = {
          id: String(m.boss.id || '').slice(0, 24), name: String(m.boss.name || '').slice(0, 40),
          health: num(m.boss.health, 100, 0, 1e6), maxHealth: num(m.boss.maxHealth, 100, 1, 1e6),
    ...
    case 'down': { p.down = true; p.health = 0; room.broadcast({ t: 'down', id: p.id, name: p.name, cause: String(m.cause || '').slice(0, 60) }); return; }
    case 'revive': {
      const target = room.players.get(m.id | 0);
      if (!target || !target.down) return;
      target.down = false; target.health = num(m.hp, 35, 1, 1000);
    ...
    case 'chat': {
      ...
      room.chat.push(line); if (room.chat.length > 60) room.chat.shift(); room.broadcast(line); return;
    }
